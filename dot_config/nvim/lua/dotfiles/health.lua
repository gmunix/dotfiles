...
function M.check()
	...
	report("info", "Run chezmoi apply --include scripts to install configured OS dependencies.")
end
...
function M.register()
	...
	if not tooling.can_install_treesitter_parsers() and vim.fn.exists(":TSUpdate") == 0 then
		pcall(vim.api.nvim_create_user_command, "TSUpdate", function()
			vim.notify(
				"nvim-treesitter is disabled because tree-sitter CLI is missing. Run chezmoi apply --include scripts, restart Neovim, then run :TSUpdate.",
				vim.log.levels.WARN,
				{ title = "dotfiles" }
			)
